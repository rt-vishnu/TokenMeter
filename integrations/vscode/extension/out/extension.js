...
async function get(path) {
    return new Promise((resolve, reject) => {
        ...
        const req = lib.request({ hostname: url.hostname, port: url.port || (url.protocol === 'https:' ? 443 : 80),
            ...
            headers: { 'Authorization': `Bearer ${getApiKey()}` } }, (res) => {
            let raw = '';
            res.on('data', (chunk) => (raw += chunk));
            res.on('end', () => {
                if ((res.statusCode ?? 0) >= 400) {
...
// ── Token reporting ───────────────────────────────────────────────────────────
async function reportUsage(model, inputTokens, outputTokens, metadata = {}) {
    if (!isEnabled())
        return;
    const key = getApiKey();
    if (!key) {
...
function hookCopilotChat(context) {
...
function hookLanguageModelAPI(context) {
    if (!('lm' in vscode))
        return;
