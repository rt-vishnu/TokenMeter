...
class RemoteApiClient {
  ...
  /// Retries [fn] up to [_maxAttempts] times with exponential backoff.
  /// Only retries on network-level failures, not HTTP error responses.
  ...
  Future<Map<String, dynamic>> estimate(EstimatePayload payload) async {
    return _withRetry(() async {
      final response = await http.post(
        ...
        body: jsonEncode({
          'model': payload.model,
          if (payload.inputTokens != null) 'input_tokens': payload.inputTokens,
